  ...
  return (
    <section className="py-12 sm:py-16 bg-white text-center text-black overflow-hidden relative z-0">
      ...
      <div className=" max-w-8xl mx-auto px-5 min-[500px]:px-10 md:px-20 w-full z-20 ">
        ...
        <div className="min-[1050px]:grid grid-cols-2 justify-center hidden gap-6 md:gap-12 mt-10 max-w-4xl mx-auto z-10">
          {steps.map((step) => (
            <div key={step.id} className="flex flex-col items-center ">
              <CircularProgress p1={step.p1} p2={step.p2} icon={step.icon} />
        ...
        {/* ⬇ Mobile/tab layout updated to 2 columns */}
        <div className="min-[1050px]:hidden grid min-[800px]:grid-cols-2 gap-9 min-[500px]:gap-5 md:gap-12 mt-7 sm:mt-10 max-w-6xl mx-auto z-10">
          {steps.map((step) => (
            <div
              key={step.id}
              className="flex min-[800px]:flex-col items-center mx-auto gap-2 min-[404px]:gap-5 font-karla"
            >
              <CircularProgress1 p1={step.p1} p2={step.p2} icon={step.icon} />
  ...
  return (
    <div className="relative w-52 h-52 mb-5 bg-[#641171]/10 rounded-full z-10">
      <svg height="100%" width="100%" viewBox="0 0 120 120" className="transform -rotate-90">
        <circle stroke="#641171" fill="transparent" strokeWidth={stroke} r={normalizedRadius} cx="60" cy="60" />
        <circle stroke="#641171" fill="transparent" strokeWidth={stroke} strokeDasharray={circumference} strokeDashoffset={strokeDashoffset2} r={normalizedRadius} cx="60" cy="60" className="transition-all duration-500" />
        <circle stroke="#f3e8ff" fill="transparent" strokeWidth={stroke} strokeDasharray={circumference} strokeDashoffset={strokeDashoffset1} r={normalizedRadius} cx="60" cy="60" className="transition-all duration-500" />
      </svg>
...
const CircularProgress1 = ({ p1, p2, icon }) => {
  ...
  return (
    <div className="relative size-20 min-[418]:size-28 min-[435px]:size-32 shrink-0 bg-[#641171]/10 rounded-full z-10">
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <img src={icon} alt="icon" className="size-9 min-[412px]:size-12" />
      </div>
    </div>
  );
};
